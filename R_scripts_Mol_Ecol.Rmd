...
uzdir <- "/file path/Manuscript_3_R_data/"
otutable_biom_file <- paste(uzdir, "113_soils_merged_otu_table.json", sep = "")
map_file <- paste(uzdir, "113_cDNA_DNA_mapping file_FINAL.txt", sep = "")
```
...
sum(taxa_sums(SS1_phyloseq) > 0)
any(taxa_sums(SS1_phyloseq)== 0)
sum(taxa_sums(SS1_phyloseq) == 0)
...
any(sample_sums(SS1_phyloseq) == 0)
any(sample_sums(SS1_phyloseq) > 0)
sum(taxa_sums(SS1_phyloseq) > 0)
...
#### Plot alpha diversity cDNA level
##### richness plots
...
####Similar results were observed for second (R2) and third replicates (R3)
...
any(sample_sums(DNA_HM_phyloseq) == 0)
any(sample_sums(DNA_HM_phyloseq) > 0)
sum(taxa_sums(DNA_HM_phyloseq) > 0)
...
any(sample_sums(DNA_LM_phyloseq) == 0)
any(sample_sums(DNA_LM_phyloseq) > 0)
sum(taxa_sums(DNA_LM_phyloseq) > 0)
...
any(sample_sums(cDNA_HM_phyloseq) == 0)
any(sample_sums(cDNA_HM_phyloseq) > 0)
sum(taxa_sums(cDNA_HM_phyloseq) > 0)
...
any(sample_sums(cDNA_LM_phyloseq) == 0)
any(sample_sums(cDNA_LM_phyloseq) > 0)
sum(taxa_sums(cDNA_LM_phyloseq) > 0)
...
write.table(sim_LM_DNA, "/file path/LM_cDNA.txt",sep="\t")
...
otus_ss <- otus@.Data
fit <- pvclust(otus_ss, method.hclust="ward", method.dist="bray–curtis", n=1000)
...
t_test<- read.csv("~/Documents/R_files/HM_rrn_t_test.csv", header=T)
t_test
...
t_test<- read.csv("~/Documents/R_files/LM_rrn_t_test.csv", header=T)
...
mn_test<- read.csv("~/Documents/R_files/HM_mn_test.csv", header=T)
mn_test
...
model.nls <- nls(Growth_Day_HM ~ Vm * rrn_copy_HM/(K+rrn_copy_HM), data = mn_test,   start = list(K = max(mn_test$Growth_Day_HM)/2, Vm = max(mn_test$Growth_Day_HM)))
...
model.nls <- nls(Max_abundance_HM ~ Vm * rrn_copy_HM/(K+rrn_copy_HM), data = mn_test,   start = list(K = max(mn_test$Growth_Day_HM)/2, Vm = max(mn_test$Max_abundance_HM)))
...
model.nls <- nls(Fold_changes_HM ~ Vm * rrn_copy_HM/(K+rrn_copy_HM), data = mn_test,   start = list(K = max(mn_test$Growth_Day_HM)/2, Vm = max(mn_test$Fold_changes_HM)))

summary(model.drm)
summary(model.nls)

###LM
LM_test<- read.csv("~/Documents/R_files/LM_mn_test .csv", header=T)
LM_test
...
model.nls <- nls(Growth_Day_LM ~ Vm * rrn_copy_LM/(K+rrn_copy_LM), data = LM_test,start = list(K = max(LM_test$Growth_Day_LM)/2, Vm = max(LM_test$Growth_Day_LM)))
...
model.nls <- nls(Max_abundance_LM ~ Vm * rrn_copy_LM/(K+rrn_copy_LM), data = LM_test,   start = list(K = max(LM_test$Growth_Day_LM)/2, Vm = max(LM_test$Max_abundance_LM)))
...
model.nls <- nls(Fold_changes_LM ~ Vm * rrn_copy_LM/(K+rrn_copy_LM), data = LM_test,   start = list(K = max(LM_test$Growth_Day_LM)/2, Vm = max(LM_test$Fold_changes_LM)))
...
best_test<- read.csv("~/Documents/R_files/best_LM_HM.csv", header=T)
best_test
...
model.nls <- nls(Growth_Day_best ~ Vm * rrn_copy_best/(K+rrn_copy_best), data = best_test,start = list(K = max(best_test$Growth_Day_best)/2, Vm = max(best_test$Growth_Day_best)))
...
model.nls <- nls(Max_abundance_best ~ Vm * rrn_copy_best/(K+rrn_copy_best), data = best_test,   start = list(K = max(best_test$Growth_Day_best)/2, Vm = max(best_test$Max_abundance_best)))
...
model.nls <- nls(Fold_changes_best ~ Vm * rrn_copy_best/(K+rrn_copy_best), data = best_test,   start = list(K = max(best_test$Growth_Day_best)/2, Vm = max(best_test$Fold_changes_best)))
...
```{r}
#### Figure 3 (DNA)
Fs3 <- read.csv("/file path/Figure_3_DNA_MS_3.csv",fill = TRUE, header = TRUE, sep = ",")
library(Rmisc)
...
pd <- position_dodge(0.001) # move them .05 to the left and right
#cols <- c("No_Urea_1.0kPa" = "red","No_Urea_10kPa" = "blue","Urea_1.0kPa" = "darkgreen", "Urea_10kPa" = "orange")
a=ggplot(tgc1, aes(x=Day, y=Abundance, group=Treatment)) + 
  ...
  #geom_point(position=pd, size=1, shape=20, fill="black") + # 21 is filled circle
  xlab("Day") +
...
#b= a+scale_color_manual(values=c("#111111", "gray70"))  #Black and gray

fig3
#output file as pdf: ggsave("/Users/Sainur/Desktop/fig3.pdf",width=7,height=9)


#Figure 5
Fs5 <- read.csv("/file path/Figure_5_DNA_MS_3.csv",fill = TRUE, header = TRUE, sep = ",")
...
fig5
#output file as pdf:ggsave("/Users/Sainur/Desktop/fig5.pdf",width=8,height=11)


#Figure 6
Fs6 <- read.csv("file path/Figure_6_DNA_MS_3.csv",fill = TRUE, header = TRUE, sep = ",")
...
##ppp= pp+scale_color_manual(values=c("#111111", "gray70"))  #Black and gray

fig6
#output file as pdf: ggsave("/Users/Sainur/Desktop/f6.pdf",width=9,height=4)
